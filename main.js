...
var OBSIDIAN_NOTE_ICON_SVG, VIEW_TYPE_CALLOUT_ORGANIZER, DEFAULT_SETTINGS, HEADING_REGEX, CALLOUT_REGEX, BLOCK_ID_REGEX, CACHE_VERSION, PLUGIN_FOLDER, CACHE_FILENAME, LUCIDE_ICON_CLASSIFICATIONS;
var init_constants = __esm({
  "modules/constants.ts"() {
    OBSIDIAN_NOTE_ICON_SVG = `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="svg-icon lucide-pencil"><path d="M21.174 6.812a1 1 0 0 0-3.986-3.987L3.842 16.174a2 2 0 0 0-.5.83l-1.321 4.352a.5.5 0 0 0 .623.622l4.353-1.32a2 2 0 0 0 .83-.497z"></path><path d="m15 5 4 4"></path></svg>`;
    VIEW_TYPE_CALLOUT_ORGANIZER = "callout-organizer";
    DEFAULT_SETTINGS = {
      excludedFolders: [],
      groupByType: false,
    ...
    BLOCK_ID_REGEX = /\^([\w-]+)\s*$/;
    CACHE_VERSION = "1.4";
    PLUGIN_FOLDER = "callout-organizer";
    CACHE_FILENAME = "callouts.json";
    LUCIDE_ICON_CLASSIFICATIONS = {
      "a-arrow-down": [
        "text",
        "design"
      ],
...
var callout_parser_exports = {};
__export(callout_parser_exports, {
  CalloutParser: () => CalloutParser
});
var CalloutParser;
var init_callout_parser = __esm({
  "modules/callout-parser.ts"() {
    init_utils();
    init_constants();
    CalloutParser = class {
      constructor(app, settings) {
        ...
        this.MAX_CONCURRENT_PROCESSING = 3;
        this.app = app;
      ...
      async scanAllCallouts() {
        ...
        const remainingFiles = files.filter(
          (file) => !processedFiles.has(file.path) && !this.shouldSkipFile(file.path, true)
        );
        await this.processFilesInBatches(remainingFiles, existingCache, callouts);
        return callouts;
      }
      async processFilesInBatches(files, existingCache, callouts) {
        const batchSize = this.MAX_CONCURRENT_PROCESSING;
        for (let i = 0; i < files.length; i += batchSize) {
          const batch = files.slice(i, i + batchSize);
          const batchPromises = batch.map(
            (file) => this.extractCalloutsFromFile(file, existingCache)
          );
          try {
...
var CacheManager = class {
  constructor(app) {
    ...
    this.incrementalUpdateQueue = /* @__PURE__ */ new Map();
    this.incrementalUpdateTimer = null;
    this.INCREMENTAL_UPDATE_DELAY = 1e3;
    this.app = app;
  }
  getCacheFilePath() {
  ...
  async processIncrementalUpdates() {
    ...
    try {
      ...
      const { CalloutParser: CalloutParser2 } = await Promise.resolve().then(() => (init_callout_parser(), callout_parser_exports));
      const parser = new CalloutParser2(this.app, {});
  ...
  async saveIncrementalCache(cache) {
    try {
...
var CalloutOrganizerView = class extends import_obsidian2.ItemView {
  ...
  // Setup lazy math processing using intersection observer
  setupLazyMathProcessing() {
      ...
      entries.forEach((entry) => {
        ...
        if (entry.isIntersecting) {
          ...
          mathElements.forEach((mathEl) => {
            this.queueMathProcessing(mathEl);
          });
          (_a = this.intersectionObserver) == null ? void 0 : _a.unobserve(element);
        }
      });
    }, {
      root: this.calloutContainerElement,
      rootMargin: "100px",
      // Start processing 100px before element becomes visible
      threshold: 0.1
    });
  }
  // Queue math processing to avoid blocking the UI
  queueMathProcessing(element) {
    this.mathProcessingQueue.push(element);
    if (!this.mathProcessingActive) {
      this.processMathQueue();
    }
  }
  // Process math elements in batches to avoid blocking UI
  async processMathQueue() {
    if (this.mathProcessingQueue.length === 0) {
      this.mathProcessingActive = false;
      return;
    }
    this.mathProcessingActive = true;
    const batch = this.mathProcessingQueue.splice(0, this.MAX_CONCURRENT_MATH);
    try {
      await Promise.all(batch.map((element) => this.processMathForElement(element)));
    } catch (error) {
      console.warn("Error processing math batch:", error);
    }
    requestAnimationFrame(() => {
      this.processMathQueue();
    });
  }
  // Math processing for MathJax integration (now returns a promise for batching)
  async processMathForElement(element) {
    try {
